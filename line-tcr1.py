...
kk = LINETCR.LINE()
kk.login(token='Elb7MsxHwS3UT0ChYgp7.Oa5sbxNtBsBfVZtKvM8HTW.cNt63AcI1XIJTORs4tEd8JoFVP4dF0ShMXOlOT8xgpc=')
kk.loginResult()
...
kd = LINETCR.LINE()
kd.login(token='Elu6BPYaIuuGP5lfvzia.AS+5ifZOG/3k0fdUVXmgMG.8QD2GRtyxmDFLiN7/gc7tcvUs4mN2T2tGn0Oj+HZokQ=')
kd.loginResult()
...
☠Respo︎n☠
☠Bot cancel☠
☠Sisca Keluar , Natalia Keluar, Kiti Keluar, Okta Keluat☠
"""
KAC=[cl,ki,kk,kc,kd]
...
wait = {
    ...
    'timeline':True,
    'autoAdd':False,
    'message':"BOT By :\n\nAldino Ft. To'if ( http://line.me/ti/p/~aldinorenaldi )",
    "lang":"JP",
    "comment":"BOT By :\n\nAldino Ft. To'if ( http://line.me/ti/p/~aldinorenaldi )",
    "commentOn":True,
    "commentBlack":{},
...
def bot(op):
    try:
            ...
            if not op.param2 in Bots:
                if wait["protectionOn"] == True: 
                    try:
                        ...
                        ki.cancelGroupInvitation([op.param3])                        
                    except Exception, e:
                       print e
        if op.type == 11:
            if not op.param2 in Bots:
        ...
        if op.type == 25:
            ...
            elif msg.text in ["All cancel","Cancel","Bot cancel"]:
                if msg.toType == 2:
              ...
              if msg.toType == 2:
                ...
                if wait["protectionOn"] == True:
                    if wait["lang"] == "JP":
                        ...
                        cl.sendText(msg.to,"Protection on bosque.")
                else:
...
#-----------------------------------------------
                ...
                if msg.toType == 2:
                    ...
                    try:
                        ...
                        pass
            elif msg.text in ["Natalia keluar"]:
                if msg.toType == 2:
                    ginfo = cl.getGroup(msg.to)
                    try:
                        kk.leaveGroup(msg.to)
...
#-----------------------------------------------
            elif msg.text in ["Tagall","Woy kontol"]:
                ...
                if msg.toType == 2:
                    print "ok"
                    _name = msg.text.replace("Cleanse","Bye kontol")
...
#-----------------------------------------------
                ...
                kc.sendText(msg.to,"bct  􀨁􀄻double thumbs up􏿿")
                kd.sendText(msg.to,"Apalu  􀨁􀄻double thumbs up􏿿")
#-----------------------------------------------
            elif msg.text in ["Member say hi"]:
                ki.sendText(msg.to,"Hi 􀜁􀅔Har Har􏿿")
                kk.sendText(msg.to,"Hay New mem 􀜁􀅔Har Har􏿿")
                kc.sendText(msg.to,"Jangan Lupa follow Twitter @AldinoRenaldi16 yah 􀜁􀅔Har Har􏿿")
                kd.sendText(msg.to,"Ga follow kick😂 ")
...
#-----------------------------------------------
                ...
                kk.sendText(msg.to,"New member wajib pap kntl")
#-----------------------------------------------
            elif msg.text in ["About","Ping","ping"]:
                ki.sendText(msg.to,"Aldino Renaldi http://line.me/ti/p/~aldinorenaldi􏿿")
                kk.sendText(msg.to,"Follow juga twitternye twitter.com/aldinorenaldi16􏿿")
                kc.sendText(msg.to,"Instagram juga follow babang instagram.com/aldinorenaldi")
                kd.sendText(msg.to,"Follow semua ya😘😘")
#-----------------------------------------------
            elif msg.text in ["Respon","respon"]:              
                ki.sendText(msg.to,"Sisca Hadir")
                kk.sendText(msg.to,"Natalia Hadir")                
                kc.sendText(msg.to,"Christy Hadir")
                kd.sendText(msg.to,"Okta Hadir")
#-----------------------------------------------
                ...
                kd.sendText(msg.to, "%sDetik" % (elapsed_time))
